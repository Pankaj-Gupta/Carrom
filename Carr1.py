#to make striker come at correct position when it comes to rest


chance = 1
pone = 0 
ptwo = 0
don = 1   #to check if player has taken backup of king or not.
nog = 0
nog1 = 0
yo = False 
done = True
...
def game():
    ...
    class Hole(pygame.sprite.Sprite):
	    ...
	    self.holesurface.set_colorkey((0,0,0))
            pygame.draw.circle(self.holesurface, (100,40,33), (30,30),30) # paint blue circle
    ...
    class Bird(pygame.sprite.Sprite):
        ...
        def __init__(self, startpos=screen.get_rect().center):
            ...
            self.ballsurface=pygame.Surface((40, 40))
	    self.ballsurface.set_colorkey((0,0,0))
            if(Bird.number == 1 or Bird.number >=20):
                pygame.draw.circle(self.ballsurface, (255,0,0), (20,20),20)
            elif(Bird.number % 2 ==0):
                pygame.draw.circle(self.ballsurface, (230,210,100), (20,20),20)
            else:
                pygame.draw.circle(self.ballsurface, (10,10,50), (20,20),20)                
        ...
        def update(self, seconds):
            ...
            global pone, ptwo, nog, nog1, don, yo
            ...
            if yo == True:                    
                    if chance != don:
                        Bird((screen.get_width()/2, screen.get_height()/2))#reappear queen
                        yo = False
                        nog1 = nog    
            ...
            if(self.pos[0]<50 and self.pos[1]<50) or (self.pos[0]<50 and self.pos[1]>(screen.get_height()-50)) or (self.pos[0]>(screen.get_width()-50) and self.pos[1]<50) or (self.pos[0]>(screen.get_height()-50) and self.pos[1]>(screen.get_height()-50)):
                nog = nog + 1
                if self.number == 1:
                    don = chance
                    nog1 = nog
                ...
                if self.number >= 2 and self.number %2 == 0:
                    if chance % 2 == 0: 
                        ...
                        pone = pone +2
                    self.kill()                        
                elif self.number >= 2 and self.number %2 != 0:
                    if chance % 2 == 0: 
                        ptwo = ptwo+1
                    else:
                        pone = pone +1
    ...
    class Goti(Bird):
        def __init__(self, startpos=screen.get_rect().center):
            Bird.__init__(self)
            self.image = pygame.Surface((50,50)) # created on the fly
            self.image.set_colorkey((0,0,0)) # black transparent
            pygame.draw.circle(self.image, (228,130,33), (25,25), 25)
            pygame.draw.circle(self.image, (10,105,50), (25,0), 4)
            self.image = self.image.convert_alpha()
            self.pos = [0,0] # dummy values to create a list
            self.pos[0] = float(startpos[0]) # float for more precise calculation
            self.pos[1] = float(startpos[1] + 290)
            self.rect = self.image.get_rect()
            self.radius = 25 # for collide check
            self.angle = 0.0
            self.speed = 50.0
        ...
        def update(self, time):
                global chance, done, done1
                pygame.display.set_caption("Chance : %i    SPEED : %i    Player1 : %i   Player2 : %i    dx : %i      dy : %i      done : %i" % ((chance+1)%2 + 1, self.speed, pone, ptwo, self.dx, self.dy, done))      
                ...
                if pressedkeys[pygame.K_RIGHT]: # right side
                         self.pos[0] = self.pos[0] + self.speed*seconds
                if pressedkeys[pygame.K_LEFT]: # left side
                         self.pos[0] = self.pos[0] - self.speed*seconds 
                ...
                '''if math.fabs(self.dx)==0 or math.fabs(self.dy)==0:
                    done = True
                else:
                    done = False

                
                if self.dx==0 and self.dy==0 and done == True and done1 == True:
                            chance += 1
                            if chance % 2 == 0:
                                self.pos[0] = 400
                                self.pos[1] = 690
    ...
    mypic = pygame.image.load("Ca.jpg")
    ...
    mainloop = True
    FPS = 200                           # desired max. framerate in frames per second. 
    ...
    while mainloop:
        ...
        '''pygame.display.set_caption("pone : %i max(ms): %i fps: %.2f birds: %i gravity: %s bad:%s clever:%s"% (milliseconds, 
